...
namespace vcp {
	...
	struct VanDerPol : public vcp::Newton< _T, _P > {
		...
		//L_botを作る
		vcp::matrix< _T, _P > L_bot() {
			Generator1.clear_data();
			Generator2.clear_data();

			Generator1.add_dpt();
			Generator2.add_dpt();
			vcp::matrix< _T, _P > L1 = Generator1.output_Jacobi();
			vcp::matrix< _T, _P > L2 = Generator2.output_Jacobi();

			vcp::matrix< _T, _P > zero1, zero2;
			zero1.zeros(L1.rowsize(), L1.columnsize());
			zero2.zeros(L2.rowsize(), L2.columnsize());

			return vercat( horzcat(L1, zero1), \
			               horzcat(zero2, L2) );
		}
		...
		vcp::matrix< _T, _P> dN() {
			...
			vcp::matrix< _T, _P > dN2 = Generator2.output_Jacobi();
			...
			vcp::matrix< _T, _P > dN3 = Generator1.output_Jacobi();
			vcp::matrix< _T, _P > dN4 = Generator2.output_Jacobi();

			vcp::matrix< _T, _P > zero1;
			zero1.zeros(dN2.rowsize(), dN3.columnsize());

			return vercat( horzcat(zero1, dN2), \
			               horzcat(dN3, dN4) );
		}

		vcp::matrix< _T, _P> Df() override {
			return this->L_bot() + this->dN();
		}

		//基底の行列L^{-1}を作成
		vcp::matrix< _T, _P> L_inv() {
			vcp::fourier_basis< _T, _P > L_base;
			vcp::matrix< _T, _P > L_inv, eye;

			eye.eye(order*2+1);

			L_base.setting_m(order);
			L_base.create_full_list();
			L_base.add_scalar_pt(1);
			L_inv = L_base.output_Jacobi();
			L_inv = lss(L_inv, eye);

			vcp::matrix< _T, _P > zero;
			zero.zeros(L_inv.rowsize(), L_inv.columnsize());

			return vercat( horzcat(L_inv, zero), \
			               horzcat(zero, L_inv) );
		}

		vcp::matrix< _T, _P> make_L() {
			vcp::fourier_basis< _T, _P > L_base;
			vcp::matrix< _T, _P > L, eye;
			...
			L_base.create_full_list();
			L_base.add_scalar_pt(1);
			L = L_base.output_Jacobi();

			vcp::matrix< _T, _P > zero;
			zero.zeros(L.rowsize(), L.columnsize());

			return vercat( horzcat(L, zero), \
			               horzcat(zero, L) );
		}
...
//       遅延項のところはn+(kx1/2)/h-Lなどを確かめないといけないので全てzで代用する.
	...
	// x,yのブロックで並んでたやつを、交互にする
	// std::vector< int > change_list( const int m_verify, const int var_num ){
	std::vector< int > change_list( const int m_verify ){
	...
	std::vector< int > reverse_change_list( std::vector< int > clist ){
		std::vector< int > list(clist.size(), 0);
	...
	template < typename _T, typename _P >
	vcp::matrix< _T, _P > change_vector( const vcp::matrix< _T, _P >& A, const std::vector< int >& clist ){
		vcp::matrix< _T, _P > B;
		B.zeros( A.rowsize(), 1 );
	...
	template < typename _T, typename _P >
	vcp::matrix< _T, _P > change_3mbotTO3m( const vcp::matrix< _T, _P >& A, int m, int n ){
		vcp::matrix< _T, _P > B;
		B.zeros( 2*(2*n+1), 1);
		int ii = 0;
		for (int i = (2*m+1)*2; i < 2*(2*n+1); i++){
			B(i, 0) = A(ii, 0);
			ii++;
		}
		return B;
	...
	template < typename _T, typename _P >
	vcp::matrix< _T, _P > inverse_change_vector( const vcp::matrix< _T, _P >& A, const std::vector< int >& clist ){
		vcp::matrix< _T, _P > B;
		B.zeros( A.rowsize(), 1 );
